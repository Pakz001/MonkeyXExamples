...
'        DebugLog offx+":"+offxt+","+offy+":"+offyt
    ...
    Method draw()
        ...
        Local d:Int=distance(cx,cy,x,y)
        DrawText 	d,
			        (x1-tilewidth)+(tilewidth/2)+20,
                    y1+tileheight/2,
                    0.5,0.5
    End Method
End Class
...
Class player
	...
	Method firetoenemy()
		Local cx:Int,cy:Int
		Local px = (p.x)/tilewidth+mapx
   		Local py = (p.y)/tileheight+mapy
		Local d:Int=10000
		For Local i:=Eachin mymonster
			Local dis:Int=distance(px,py,i.x,i.y)
			If dis < d
				d=dis
				cx = i.x
				cy = i.y
		...
		If d<10
			shots.AddLast(New shot(px,py,cx,cy))
			DebugLog Millisecs()
		End If
	End Method
End Class
...
Class shot
	...
	Method New(tx:Int,ty:Int,x:Int,y:Int)
		Self.x = tx*tilewidth
		Self.y = ty*tileheight
		Self.tx = (x*tilewidth)+Rnd(-20,20)
		Self.ty = (y*tileheight)+Rnd(-20,20)
		Self.angle = getangle(Self.tx,Self.ty,Self.x,Self.y)
	End Method
	Method update()
		x+=Cos(angle)*4
		y+=Sin(angle)*4
		...
		If timeout<0 
			remove=True
		End If
		If rectsoverlap(x,y,10,10,tx,ty,10,10)
			remove=True
		End If
	End Method
	Method draw()
...
Global shots:List<shot> = New List<shot>
